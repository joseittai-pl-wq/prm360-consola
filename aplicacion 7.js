...
async function viewAccesos(c){
  ...
  c.innerHTML='<h1 class="pg">Accesos y vinculación</h1><div class="pgsub">'+d.length+' usuario(s) registrado(s)'+(pend?(' · '+pend+' por asignar'):'')+'. Asigna rol y departamento, y vincula cada cuenta con su ficha real.</div>'+
    ...
    '<div class="card" style="margin-top:14px"><h3>Dar de alta a un nuevo usuario</h3><div class="body">'+altaForm+'</div></div>'+
  ...
  if(altaOpenBtn) altaOpenBtn.onclick=()=>{ st.showAlta=true; viewAccesos(c); };
  ...
  if(altaGuardarBtn) altaGuardarBtn.onclick=async()=>{
    ...
    altaGuardarBtn.disabled=true; msg.textContent='Guardando…'; msg.style.color='var(--muted)';
    const {data:res,error:e}=await sb.rpc('admin_crear_alta',{p_email:email,p_nombre:c.querySelector('#alta_nombre').value.trim(),p_rol:c.querySelector('#alta_rol').value,p_departamento:c.querySelector('#alta_dep').value,p_unidad:c.querySelector('#alta_uni').value});
    altaGuardarBtn.disabled=false;
    if(e || res!=='ok'){
      const map={ya_registrado:'Ese correo ya está registrado — asígnale el rol directamente en la tabla de arriba.', falta_email:'Falta el correo.'};
      msg.textContent = e? ('Error: '+e.message) : (map[res]||('No se pudo: '+res));
...
function renderAltaForm(deptos, ofis){
  ...
  return '<div class="frm">'+
    ...
    '<div style="font-size:12px;color:var(--muted);margin-top:6px">Cuando esta persona entre a <b>consola.prm360.com.mx</b> y se registre con este mismo correo, le caerá automáticamente el rol y departamento que elijas aquí — ya no queda "pendiente" esperando que tú lo asignes después.</div>';
}
